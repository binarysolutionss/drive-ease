...
export default function Hero() {
  return (
    <section className="relative h-max flex overflow-hidden bg-[#b0b0b0] pt-32">
      {/* Background video */}
      <video
      ...
      {/* Content */}
      <div className="relative z-20 w-full px-12 pb-18 grid grid-cols-1 md:grid-cols-[1fr_auto] self-start gap-8">
        {/* Left */}
        <div>
          {/* Kicker */}
          <p className="gap-2 text-sm font-medium tracking-[0.26em] uppercase text-black mb-5 animate-fade-up">
          ...
          {/* Subtitle */}
          <p className="text-sm font-semibold leading-[1.7] text-black max-w-[46ch] mb-10 animate-fade-up [animation-delay:300ms]">
            Handpicked vehicles. Transparent pricing. Zero hidden fees. Whether
            it&apos;s a weekend trip or a long-haul journey — we&apos;ve got the
            perfect ride waiting.
          </p>
          ...
          <div className="flex flex-col sm:flex-row max-w-155 bg-white/10 backdrop-blur-md border border-white/20 rounded-lg overflow-hidden animate-fade-up [animation-delay:450ms]">
            {[
              ...
              >
                <span className="block text-sm font-medium tracking-[0.2em] text-black/75mb-1">
                  {field.label}
                </span>
                <span className="font-syne text-sm font-semibold text-black">
                  {field.value}
                </span>
              </div>
            ))}
            <button className="font-syne text-xl font-bold tracking-wide bg-white text-[#1a1a1a] px-8 py-4 hover:bg-white/90 transition-colors whitespace-nowrap">
              Search →
            </button>
          </div>
        </div>
        ...
        <div className="hidden md:flex flex-col gap-6 text-right animate-fade-left">
          {[
            ...
            <div key={i}>
              ...
              <span className="font-syne block text-5xl font-extrabold text-gray-700 leading-none">
                {stat.num}
              </span>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 hidden md:flex flex-col items-center gap-2">
        <span className="text-[0.55rem] tracking-[0.25em] uppercase text-white/40">
          Scroll
        </span>
